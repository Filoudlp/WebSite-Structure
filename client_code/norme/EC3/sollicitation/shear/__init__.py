from ._anvil_designer import shearTemplate
from anvil import *
import anvil.server
...
class shear(shearTemplate):
  ...
  def __init__(self, **properties):
    ...
    self.geometrie_pou_1.txb_A.visible = False
    self.geometrie_pou_1.lbl_a.visible = False
  ...
  def btn_calc_click(self, **event_args):
    ...
    API_URL = "/api/cm_shear_calc"
    fy = self.materiaux_cm_1.steel_grade_ddm.selected_value[0][1]
    payload = {
      "fy": float(fy[1:]),
      "gamma_m0": float(self.option_avancer_cm_1.txb_gamma_m0.text),
      "Av": float(self.geometrie_pou_1.txb_Av.text),
      "load": norme.convert_unit(float(self.effort_tranchant_1.txb_V.text), "kN", "N"),
    }
    response = norme.api_call(API_URL, payload)

    print(response)

    formula = response["vpl_rd"]["formula"]
    formula_val = response["vpl_rd"]["formula_values"]
    ref = response["vpl_rd"]["ref"]
    vpl_rd = norme.convert_unit(response["vpl_rd"]["result"], "N", "kN")
    verif = response["verif"]["formula_values"]
    verif_ref = response["verif"]["ref"]
    ved = float(self.effort_tranchant_1.txb_V.text)
    ...
    self.rslt_cm_1.lbl_elu_percent.text = f"Taux : {ved / vpl_rd * 100:.2f} %"
